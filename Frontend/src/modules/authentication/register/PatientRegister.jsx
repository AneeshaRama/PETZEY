...
const PatientRegister = () => {
  ...
  const { ownerName, username, userPassword, phone, email, address, prefix } =
    userdata;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios
        .post(
          `${process.env.REACT_APP_API}/auth-service/pet-owner/register`,
          userdata
        )
        .then((data) => {
  ...
  const uploadImage = async () => {
    ...
    const imageRef = ref(storage, `images/pet-owners/${image.name + v4()}`);
